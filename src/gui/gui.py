...
GUI by Aleksandr Savinov"""
    ...
    output = Text(mainframe, background="white", font=basefont, height=code['height']/2, state="disabled")
    ...
    def __init__(self):
        ...
        ttk.Label(self.mainframe, text='Code:', justify='left', width=10).grid(row=0, column=0, sticky=(W))
    ...
    def newfile(self):
        if self.saved==False:
            val=messagebox.askyesnocancel("File not saved", "The current file has changes, that are not saved. Do you want to save?")
            if val==None:
                return
            if val:
                self.savefile()
    ...
    def openfile(self):
        if self.saved==False:
            ...
            if val:
                self.savefile()
                return
        self.file_ = filedialog.askopenfilename(defaultextension=".BAS", filetypes = [("All Files", "*.*"), ("QBASIC code", "*.BAS")])
